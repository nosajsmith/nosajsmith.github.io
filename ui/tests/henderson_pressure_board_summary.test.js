...
test("henderson pressure board stays truthful when the local perimeter picture is unavailable", () => {
  ...
  assert.match(summary.note, /unavailable outside the current perimeter slice/i);
  ...
  assert.match(summary.engagementSummary.note, /does not expose local Henderson perimeter areas/i);
  ...
  assert.match(summary.responseReadiness.note, /does not expose local Henderson perimeter areas/i);
  ...
  assert.match(summary.counterattackPlanning.note, /does not expose local Henderson perimeter areas/i);
  ...
  assert.match(summary.defensePreparation.note, /does not expose local Henderson perimeter areas/i);
  assert.deepEqual(summary.defensePreparation.areas, []);
  assert.equal(summary.localSustainment.status, "Unavailable");
  assert.match(summary.localSustainment.note, /unavailable because the active scenario does not expose the Henderson perimeter picture/i);
  assert.equal(summary.airSupport.availability, "Unavailable");
  assert.match(summary.airSupport.note, /unavailable because the active scenario does not expose the Henderson perimeter picture/i);
  assert.equal(summary.navalSupport.availability, "Unavailable");
  assert.match(summary.navalSupport.note, /unavailable because the active scenario does not expose the Henderson perimeter picture/i);
